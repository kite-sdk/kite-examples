...
if [ -z "${SPARK_HOME}" ]; then
  ...
  elif [ -d "/opt/cloudera/parcels/CDH/lib/spark" ]; then
    SPARK_HOME=/opt/cloudera/parcels/CDH/lib/spark
  fi
fi
debug "Using SPARK_HOME=${SPARK_HOME}"
...
# Add Spark to dependency list
if [ -e "$SPARK_HOME/bin/compute-classpath.sh" ]; then
  KITE_CLASSPATH=`$SPARK_HOME/bin/compute-classpath.sh`:${KITE_CLASSPATH}
fi
